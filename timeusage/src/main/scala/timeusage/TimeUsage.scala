...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val othersPrefix = List("t02", "t04", "t06", "t07", "t08", "t09",
      "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    val primary, work, others = ListBuffer[String]()

    columnNames.foreach( name => {
      if(primaryPrefix.exists(name.startsWith(_))) primary += name
      else if(workPrefix.exists(name.startsWith(_))) work += name
      else if(othersPrefix.exists(name.startsWith(_))) others += name
    })

    def listBufferToColumn(listBuffer: ListBuffer[String]): List[Column] = {
      listBuffer.toList.map(col(_))
    }

    (listBufferToColumn(primary), listBufferToColumn(work), listBufferToColumn(others))
  }
  ...
  def timeUsageSummary(
    ...
    val otherProjection: Column = otherColumns.reduce(_ + _).divide(60).as("others")
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed
      ...
      .agg(round(avg("primaryNeeds"),1).as("primaryNeeds"), round(avg("work"),1).as("work"),  round(avg("others"), 1).as("others"))
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"select working, sex, age, round(avg(primaryNeeds),1) as primaryNeeds, round(avg(work),1) as work, round(avg(others),1) as others from $viewName group by working, sex, age order by working, sex, age"
